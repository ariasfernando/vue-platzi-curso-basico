      ...
      <div v-if="plugin.enabled" class="form-group" v-for="(option, name) in plugin.config" :key="name">
        ...
        <div class="col-sm-5">
          <span>
            ...
            <input v-if="option.type === 'text'" :type="option.type" :disabled="!enabled" :value="option.value" :name="name" @change="(newValue)=>updateField(newValue, name)">
  ...
  export default {
    ...
    methods: {
      ...
      updateSubFieldByEvent(e) {
        ...
        this.updateSubField(value, option, subOption);
      },
