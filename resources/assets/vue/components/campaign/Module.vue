<template>
  <tr v-if="module.type === 'custom'"
    ...
    <td
      ...
      :width="_.get(module,'structure.attribute.width','100%')"
      :style="_.get(module,'structure.style')"
      :valign="_.get(module,'structure.attribute.valign','top')"
      :bgcolor="_.get(module,'structure.attribute.bgcolor')"
      @mousedown="onClickModule">
      <component :is="'custom-' + module.key" :module="module" :module-id="moduleId" />
      <module-toolbar v-if="!module.data.hideToolbar" :module-id="moduleId" />
      <div class="st-remove-element module-overlay" />
      <div class="st-remove-element default-module-error" />
    </td>
  </tr>
  ...
  <tr
    ...
    <td
      ...
      <background-image
        :key="modulebackgroundImage"
        :element="module.structure"
        :width="templateInnerWidth"
        :module="module"
        :module-id="moduleId">
...
module.exports = {
  ...
  computed: {
    ...
    hasErrors() {
      return (
        this.module.data &&
        this.module.data.errors &&
        this.module.data.errors.length
      );
    },
    ...
    styleModule() {
      ...
      if (this.modulebackgroundImage) {
        styleModule = this.elementBorderHorizontalPaddingAndHeight(
          this.module.structure,
        );
        styleModule = _.merge(
          styleModule,
          this.elementBackground(this.module.structure),
        );
      } else {
  ...
  methods: {
    onClickModule() {
      ...
      this.$store.commit(
        'campaign/setCurrentModuleIdInstance',
    ...
    setModulesMouseOver(e) {
      ...
      if (!$row.find('#moduleHighlight').length && $row.height() < 5) {
        const $hoverTable = $(
          '<table id="moduleHighlight"><tr><td></td></tr></table>',
        );
