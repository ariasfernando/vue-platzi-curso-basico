...
tinymce.PluginManager.add('sttextcolorextended', function(editor) {
	var cols, rows;
	...
	function mapColors() {
		...
		if(editor.settings.textcolor_categorizable) {
			return colorMap;
		} else {
	...
	function renderColorPicker() {
		...
		// if is categorizable, colors must to be an object
		if(editor.settings.textcolor_categorizable) {

			html = '<table class="mce-grid mce-grid-border mce-colorbutton-grid" role="list" cellspacing="0"><tbody>';

			var categories = _.keys(colors);

			for (c = 0; c < categories.length; c++) {
				
				category_colors = _.toArray(_.values(_.pick(colors,categories[c]))[0])

				html += '<tr>';
					html += '<td><strong>' + categories[c] + '</strong></td>';
					cols = category_colors.length;
					...
					for (x = 0; x < cols; x++) {
						...
						if (x >= cols) {
							...
							html += getColorCellHtml(color, color);
						}
					}
