...
function html2canvas(nodeList, options) {
    ...
    /*
     * FB Improvement
     * Used to scale canvas.
     */
    options.scaleRatio = typeof(options.scaleRatio) === "undefined" ? 1 : options.scaleRatio;
...
function renderWindow(node, container, options, windowWidth, windowHeight) {
    ...
    return parser.ready.then(function() {
        ...
        if (options.type === "view") {
            ...
            // Duplicate bounds by scaleRatio
            canvas = crop(renderer.canvas, {width:  options.width != null ? options.width : bounds.width * options.scaleRatio, height: options.height != null ? options.height : bounds.height * options.scaleRatio, top: bounds.top * options.scaleRatio, left: bounds.left * options.scaleRatio, x: clonedWindow.pageXOffset, y: clonedWindow.pageYOffset});
        }
...
NodeParser.prototype.renderTextDecoration = function(container, bounds, metrics) {
    ...
    case "underline":
        ...
        if( Application.utils ){
          var browser = Application.utils.getNavigator();
          var underlineTop = bounds.top + metrics.baseline + metrics.lineWidth;
          var underlineWeight = 1;

          switch ( browser[0] ){
            case "Firefox":
              underlineTop = underlineTop+6;
            break;
            case "Chrome":
              underlineTop = underlineTop+1;
              underlineWeight = 3;
              if (metrics.baseline < 18){
                underlineWeight = 1;
              }
              else if (metrics.baseline < 28 ) {
                underlineWeight = 2;
              }
            break;
          }
        }
        this.renderer.rectangle(bounds.left, Math.round( underlineTop ), bounds.width, underlineWeight, container.color("color"));
        break;
    case "overline":
...
function CanvasRenderer(width, height) {
    ...
    if (!this.options.canvas) {
        this.canvas.width = width * this.options.scaleRatio;
        this.canvas.height = height * this.options.scaleRatio;
    }
    this.ctx = this.canvas.getContext("2d");

    /*
     * FB Improvement
     * Canvas scale to improve image quality
     */
    if( this.options.scaleRatio > 1 ){
      this.ctx.scale(this.options.scaleRatio,this.options.scaleRatio);
    }
