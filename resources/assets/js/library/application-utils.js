...
Application.utils = {
    ...
    display(title, message, type) {
      ...
      if ($errorBox.length) {
        ...
        $errorBox
                    ...
                    .find('.alert').removeClass((index, css) => (css.match(/(^|\s)alert-\S+/g) || []).join(' '))
        ...
        $errorBox.slideDown(() => {
          ...
          if (!window.location.pathname == '/admin/user') {
            ...
            Application.utils.alert.timeOut = setTimeout(() => {
              if ($errorBox.is(':visible')) {
  ...
  confirm(options) {
    ...
    this.display = function () {
      const confirm = this;
      const $confirmModal = $(`#${confirm.confirmOptions.confirmModalId}`);
      ...
      $confirmModal.find('.btn-cancel').text(confirm.confirmOptions.labels.cancelBtn);
            ...
            // Set onSubmit event
      $confirmModal.find('.btn-submit').one('click', () => {
        confirm.confirmOptions.onSubmit();
        $confirmModal.modal('hide');
      });

            // Set onClose event
      $confirmModal.find('.close').one('click', () => {
        confirm.confirmOptions.onClose();
        $confirmModal.modal('hide');
      });

            // Set on escape key press
      $confirmModal.keydown((e) => {
        if (e.keyCode == 27) {
      ...
      if (!confirm.confirmOptions.noCancel) {
            // Set onCancel event
        $confirmModal.find('.btn-cancel').one('click', () => {
  ...
  spinner() {
    ...
    $modalSpinner.on('hidden.bs.modal', () => {
  ...
  validate: {
    ...
    validateEmailFormat(field) {
      const emails = field.value.split(';').filter(Boolean);
      const re = /^(([\w-\+]+(\.[^<>()\[\]\\.,;:\s@"]+)*)|(".+"))@([a-zA-Z\-0-9]+\.)+[a-zA-Z]{2,}$/i;
    ...
    parseUrl(url) {
      ...
      const result = url.search(new RegExp(/^http/i));
      if (result) {
        return `http://${url}`;
      }
    },
    ...
    validateCompareField(field, comparedField) {
    ...
    validateFileType(field) {
      ...
      const imgAllowedExtension = Application.globals.imageAllowedExtensions;
    ...
    validateFileSize(field) {
      this.initField(field);
      const imageSizeLimit = Application.globals.imageUploadSizeLimit;
    ...
    initField(field) {
      ...
      const $labelMessage = $(field).next('label');
    ...
    validateField(field) {
      ...
      const validationParams = $field.data('validation');
    ...
    validateForm(form) {
      ...
      const inputs = $(form).find('*[data-validation]');
      let errors = false;

            // Check each input.
      $.each(inputs, (key, field) => {
        const validationResult = validate.validateField(field);
  ...
  processQueue: {
    getJobStatus(jobId, callback, failCallback, run) {
      if (!jobId) {
        return false;
      }
      ...
      const ajaxRequest = Application.utils.doAjax(`/queue/status/process/${jobId}`, { type: 'GET' });
  ...
  encodeHtmlEntities($cleanedHtml) {
    ...
    $.map(all, (el, index) => {
      const textConnvert = Application.utils.charConvertHtmlEntities($(el).html());
  ...
  changeBuildingMode(selected) {
    const $canvas = Application.utils.getCanvas();
    const arrayMqClass = ['display-mobile', 'show-img-device', 'hidden-device', 'element-block-center', 'mobile-margin', 'full-width'];

    $canvas.trigger('changeBuildingMode', [selected]);

    $.each(arrayMqClass, (key, name) => {
      if (selected == 'mobile') {
  ...
  replaceRgbWithHex($cleanedHtml) {
    const all = $cleanedHtml.find('*');
    const rgbRegex = /^rgb\(\s*(-?\d+)(%?)\s*,\s*(-?\d+)(%?)\s*,\s*(-?\d+)(%?)\s*\)$/;
    $.map(all, (el, index) => {
          ...
          $(el)
                        .attr('style',
                            ...
                            .replace(
                                ...
                                ($0, $1, $2, $3) => `#${(`0${Number($1).toString(16)}`).substr(-2)}${(`0${Number($2).toString(16)}`).substr(-2)}${(`0${Number($3).toString(16)}`).substr(-2)}`),
                        );
        }
      }
    });
  ...
  isTooDark(hexVal, tolerance) {
    var tolerance = tolerance || 40;
    const c = hexVal.substring(1); // strip #
  ...
  preheaderWarning(el) {
    ...
    if ($(el).val().length > 0) {
      const validationResult = Application.utils.validate.validateField(el);
      if (!validationResult.success) {
  ...
  cssToObj(css) {
    ...
    let obj = {},
        s = css.replace(/-(.)/g, (m, g) => `-${g.toLowerCase()}`).replace(/;\s?$/g, '').split(/:|;/g);
