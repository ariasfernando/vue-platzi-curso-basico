@foreach ($logs as $log)
	<tr data-log="{{ $log->id }}">
		...
		<td title="{{ ( isset($log->properties["user_id"]))? $log->properties["user_id"] : "" }}">
			@if ( isset($log->properties["user_id"]) )
				<a href="{{ action('Admin\UserController@getIndex', array("type" => "_id", "q" => $log->properties["user_id"]->{'$id'})) }}" target="_self">
