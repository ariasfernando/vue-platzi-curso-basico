  ...
  <SettingsContainer custom-class="generic-color" :label="plugin.title">
    <template slot="setting-right">
      ...
      <Modal
        ...
        buttonSubmitText="Import values"
...
import Vue from 'vue';
import EntryAdapter from './entry-adapter';
...
export default {
  ...
  computed: {
    elements() {
      return _.get(this.module, 'structure.columns', []).reduce((ret, column) => {
        ret.push(column);
        return _.get(column, 'components', []).reduce((ret, component) => {
          ret.push(component);
          return ret;
        }, ret);
      }, [])
