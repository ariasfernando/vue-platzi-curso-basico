...
class AssignPermissionStudio extends Migration
{
    ...
    private function buildPermissions()
    {
        ...
        $advancedPermissions = $this->allPermissions;
        unset($advancedPermissions['std_raw']);

        // Remove permissions for basic user
        $basicPermissions = $this->allPermissions;
        unset($basicPermissions['std_raw']);
        unset($basicPermissions['std-plugin-text-color-by-background-for-module']);
        unset($basicPermissions['std-divider-element-border-group']);

        $roles_permissions = [
            'studio-admin' => $this->allPermissions,
