...
class Eloqua implements ApiConnector
{
     ...
     */
    private function getToken($params = [], $force = false)
    {
    ...
    private function getBaseUrlByOauth()
    {
        ...
        if (Session::has('api:eloqua:url')
            ...
            Session::put('api:eloqua:url:expires_in', Carbon::now()->addHours(1)->toDateTimeString());
        }
    ...
    public function oauth($request)
    {
        if ($request->session()->has('api:eloqua:token')
            ...
            if ($code = $request->input('code')) {
                ...
                $request->session()->put('api:eloqua:token', $access_token);
            } else {
